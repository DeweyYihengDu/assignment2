...
The 95% prediction intervals are wide. LnRR are expected to range from -1.0981 to 0.3142 95% of the time with repeated experiments, suggesting a lot of inconsistency between studies.

* The 95% confidence interval ranged from -1.0981 to 0.3142.
* In 95% of the probability cases, the true value is expected to fall from -1.0981 to 0.3142.
...
* From the model we find that no total variation in effect size estimates is the result of differences between studies, because their $I^2 $ is close to 0.
* From the multilevel meta-analysis model, we found that only 1.73% of the total variation in effect size estimates was the result of inter-species variation.
* But about 21% of the total variation in effect size estimates was the result of Year_online variation.
...
```{r, fig.cap="Fig1 Orchard plot of the main model"}
...
```{r, fig.cap="Fig2 Orchard plot about the Climate"}
...
```{r, fig.cap="Fig3 Orchard plot of the meta-regression model about Life Stage"}
...
```{r, fig.cap="Fig4 Orchard plot of the meta-regression model about effect type"}
...
```{r, fig.cap="Fig5 Funnel plot showing the precision of effects against their correlation"}
...
```{r, fig.cap="Fig 6 The plot of Sampling Variance of LnRR against LnRR"}
...
```{r, fig.cap="Fig7 Plot of LnRR as a function of publication year"}
...
We can see that sampling variance explains 31.32% of effect size variance. This is the marginal $R^2$, which tell us how much variation the ‘fixed effects’ or moderators explain in the model. Conditional $R^2$ tells us that the full model, that accounts for the both the fixed and random effects, explains 72.14% of variance in effect size.
